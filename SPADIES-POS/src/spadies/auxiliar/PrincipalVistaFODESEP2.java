...
public class PrincipalVistaFODESEP2 {
  ...
  public static void main(String[] args) throws Exception {
    ...
    File fBeneT = new File(carBaseVFOD,"benef2.csv");
    final File fBenefVarios = new File(carBaseVFOD,"sintesisapoyosfin2007_08.csv");
    final File fBenefVariosPro = new File(carBaseVFOD,"pros/sintesisapoyosfin2007_08.csv.procesado");
    final File fBenefVariosInd = new File(carBaseVFOD,"sintesisapoyosfin2007_08.indcsv");
    final File fBenefTotal = new File(carBaseVFOD,"benef3.csv");
    ...
    final File fSisbenBeneficiarios = new File(carBaseVFOD,"datBEN_SIS.out.csv"),
      fSisbenBeneficiariosPro = new File(carBaseVFOD,"pros/datBEN_SIS.out.procesado"),
      fSisbenBeneficiariosInd = new File(carBaseVFOD,"datBEN_SIS.out.csv.indcsv");
    final File fECAES = new File(carBaseVFOD,"ecaes_sint2004a2006reduc.csv"),
      fECAESPro = new File(carBaseVFOD,"pros/ecaes_sint2004a2006reduc.procesado"),
      fECAESInd = new File(carBaseVFOD,"ecaes_sint2004a2006reduc.csv.indcsv");
    final File fObs = new File(carBaseVFOD,"graduado_base.csv"),
      fObsPro = new File(carBaseVFOD,"pros/graduado_base.procesado"),
      fObsInd = new File(carBaseVFOD,"graduado_base.csv.indcsv");
    ...
    final File fSisbenBeneficiariosObsGrad = new File("datFOD_BEN/datBEN_SIS_OBS_GRA.out.csv"),
      ...
      */
    final File fSisbenBeneficiariosRBase = new File(carBaseVFOD,"datBEN_SIS_SPAR.out.csv");
    ...
    if (!fBenefTotal.exists()) {//Match Beneficiarios-BeneficiarioOtros
      ArchivoCSV arsis = new ArchivoCSV() {
        int conteo = 0;
        public String getTitulo() {return "BeneficiariosOtros";}
        public File getIn() {return fBenefVarios;}
        public File getOut() {return fBenefVariosPro;}
      ...
      if (!fBenefVariosInd.exists()) indexarCSV(fBenefVarios, fBenefVariosInd);
      int regs = registrosEnArchivo(fBeneT);
      ...
      PrintStream ps = new PrintStream(fBenefTotal);
      {
        System.out.println("BeneficiariosOtros: Realizando Match");
        ...
        System.out.println("BeneficiariosOtros: Match Completado");
        System.out.println("BeneficiariosOtros: Cargando Indice");
        int[] indsis = cargarIndiceCSV(fBenefVariosInd);
        boolean[] visitados = new boolean[indsis.length];
        Arrays.fill(visitados, false);
        System.out.println("BeneficiariosOtros: Indice Cargado");
        RandomAccessFile raf = new RandomAccessFile(fBenefVarios, "r");
        raf.seek(0);
    ...
    if (!fSisbenBeneficiarios.exists()) {//Match SISBEN-Beneficiarios
      ...
      int regs = registrosEnArchivo(fBenefTotal);
      System.out.println("Registros: "+regs);
      byte[][][] INFO_IES = new byte[3][regs][];
      {
        BufferedReader br = new BufferedReader(new FileReader(fBenefTotal));
        br.readLine();
        int i = 0;
        for (String s = br.readLine();s!=null;s = br.readLine()) {
          String sL[] = csvToString(s,0,';');
          INFO_IES[0][i]=codifLetrasServer.getCodigos(sL[3].length()+sL[4].length()>sL[24].length()?sL[3] + sL[4]:sL[24]);
          INFO_IES[1][i]=codifNumeros.getCodigos(sL[5].length()>sL[23].length()?sL[5]:sL[23]);
          INFO_IES[2][i]= new byte[0];//TODO deberia
          ++i;
        }
        br.close();
    ...
    if (!fSisbenBeneficiariosRBase.exists()){//Match SISBEN_Benef con SPADIES
      ...
      ArchivoCSV arBENSIS = new ArchivoCSV() {
        ...
        public String[] getIdLinea(String[] w) {
          int maxlN = Math.max(w[3].length()+w[4].length(), Math.max(w[30].length(), w[361].length()));
          int maxlD = Math.max(w[5].length(), Math.max(w[29].length(), w[127].length()));
          return new String[]{w[30].length()==maxlN?w[30]:(w[361].length()==maxlN?w[361]:w[3] + " " + w[4]),
              maxlD==w[5].length()?w[5].trim():(w[29].length()==maxlD?w[29]:w[127])};
        ...
        for (String s = br.readLine();s!=null;s = br.readLine()) {
          ...
          if (sL[0].length()>0 && sL[1].length()>0) {
            ...
            INFO_IES[0][i]=codifLetrasServer.getCodigos(
                ...
                stringMasLargo(sL[5]+" "+sL[6], sL[32], sL[363])
            );
            //INFO_IES[1][i]=codifNumeros.getCodigos(sL[7].trim().length()==0?sL[41]:sL[7]);
            INFO_IES[1][i]=codifNumeros.getCodigos(stringMasLargo(sL[7], sL[31], sL[129]));
            INFO_IES[2][i]= new byte[0];//TODO deberia            
          }
          ++i;
        }
