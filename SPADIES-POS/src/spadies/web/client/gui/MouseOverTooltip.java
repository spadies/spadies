 ...
 */
public class MouseOverTooltip implements MouseOutHandler,MouseOverHandler {
  ...
  private static class Tooltip extends PopupPanel {
    ...
    public Tooltip(Widget sender, int offsetX, int offsetY, 
      ...
      HTML contents = new HTML("<div>"+text+"</div>");
      add(contents);
  ...
  public MouseOverTooltip(String text, int delay) {
    this(text, delay, DEFAULT_TOOLTIP_STYLE);
  }

  public MouseOverTooltip(String text, int delay, String styleName) {
    this.text = text;
    this.delay = delay;
