...
public final class PreparadorDatos {
  ...
  public void prepararArchivosBase(boolean[] tablasGrandesEncontradas) throws Exception {
    ArchivoCSV arr[]={
      ...
      new ArchivoCSV() {
        public String getTitulo() {return "SABER PRO";}
        public File getIn() {return ConstantesServer.fSBPRO;}
        public File getOut() {return ConstantesServer.gSBPRO;}
        public String[] getIdLinea(String[] w) {return new String[]{w[0],w[2]};} //Nom, doc
        public byte[] getDatoLinea(String[] w) {return prepararSalidaSBPRO(w);}
      }
    };
    for (int i=0; i<arr.length; i++) tablasGrandesEncontradas[i]=(arr[i].getOut().exists()||arr[i].getIn().exists());
    ...
    for (int i=0; i<arr.length; i++) if (tablasGrandesEncontradas[i]) prepararArchivoBase(arr[i]);
    for (int i=0; i<arr.length; i++) if (!tablasGrandesEncontradas[i]) {
  ...
  private static byte[] prepararSalidaSBPRO(String[] w) {
    MyByteSequence res=new MyByteSequence(5);
    res.setByte(0,(byte)(toByte(w[4],-1,1,100)+1));          // PuntajeSBPRO
    res.setByte(1,(byte)(toByte(w[6],-1,0,120)+1));     // Edad
    res.setBits4(2,0,(byte)(toByte(w[8],-1,1,10)+1)); //Area Con.
    res.setBits4(2,4,(byte)(toByte(w[9],-1,1,4)+1)); //edu_madre
    res.setBits4(3,0,(byte)(toByte(w[10],-1,1,5)+1)); //ocu_madre
    res.setBits2(3,4,(byte)(toByte(w[11],-1,0,1)+1));    // sbpro_poseeintern
    //Lote 3,6 (2)
    res.setBits4(4,0,(byte)(toByte(w[12],-1,0,5)+1));    // sbpro_vlrmatrant
    //Lote 4,4 (4)
    return res.getBytes();
  }
