...
public class PrincipalMatchImpactoTRC {
  ...
  public static void main(String[] args) throws Exception {
    ...
    final File fEncuesta = new File(carSisben, "encuestaTRC.csv");
    ...
    {//Match SISBEN-Encuesta
      int regs = registrosEnArchivo(fEncuesta);
      System.out.println("Registros encuesta: " + regs);
      byte[][][] INFO_IES = new byte[3][regs][];//Lazye :P
      {
        BufferedReader br = new BufferedReader(new FileReader(fEncuesta));
        br.readLine();
        int i = 0;
        for (String s = br.readLine();s!=null;s = br.readLine()) {
          String sL[] = csvToString(s,0,';');
          INFO_IES[0][i]=codifLetrasServer.getCodigos(
              sL[1] + (sL[2].length()!=0?" " + sL[2]:"") + " " +
              sL[3] + (sL[4].length()!=0?" " + sL[4]:"")
           );
    ...
    fos.close();
    System.out.println("Proceso completado en: " + (System.currentTimeMillis()-t0)/1000);
  }
