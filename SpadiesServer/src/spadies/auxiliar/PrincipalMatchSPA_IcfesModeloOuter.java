...
public class PrincipalMatchSPA_IcfesModeloOuter {
  ...
  public static void main(String[] args) throws Exception {
    ...
    final File fICFESToPro = new File(carDatosNoSPA, "icfes.procesado");
    final File fICFESToInd = new File(carDatosNoSPA, "icfes.ind");
    ...
    final File fSalida = new File("salidaModeloFullIcfes.csv");
    /*{
    ...
    {//Match con ICFES, SISBEN
      ...
      {//Impresion Salida
        ...
        System.out.println("ICFES: Cargando indices");
        long[] indICF = cargarIndiceCSV(fICFESToInd);
        //long[] indSIS = cargarIndiceCSV(fSisbenToInd);
        System.out.println("ICFES: Indices cargados");
        String[] enc0 = encSPA/*,
          enc3 = encSPA*/;
        ...
        int ic = 0;
        boolean[] visitados = new boolean[indICF.length];
        ...
        for (int i = 0, it = visitados.length;i < it;i++) {
          ...
          rafICF.seek(indICF[i]);
          String[] dat1 = splitCSV(rafICF.readLine());
          ps.println(stringToCSV(dat1) + stringToCSV(dat0v));
          ++noVis;
        }
        System.out.println("ICFES no en SPADIES " + noVis);
        ps.close();
        rafICF.close();
