...
public class PanelDesercionPorCohorteTajada extends JPanel implements ActionListener {
  ...
  public PanelDesercionPorCohorteTajada() {
    for (int i=0; i<tam; i++) {
  ...
  private String getStringSeleccion() {
    ...
    return RutinasGUI.getStringSeleccion("<p>Permite comparar para cada cohorte, el comportamiento de la tasa de deserción por cohorte en un periodo determinado.</p><p>",(MyPanelSeleccion[])(params[0]),(JCheckBox[])(params[1]),(Variable[])(params[2]),null,VentanaPrincipal.getInstance().estaSeleccionandoAgregado());
  }
  public void actionPerformed(ActionEvent e) {
    ...
    private void generarGrafica() throws Exception {
      ...
      for(Entry<byte[],double[][]> vals:mvals.entrySet()) {
        ...
        for (byte i:semPer) {
          ...
          for (int j=0; j<tam; j++,fil++) {
            serie[j] = ser[j][i-1]==Double.MAX_VALUE?Double.NaN:ser[j][i-1]/100d;//TODO revisar si en el kernel hay mucha vaina que este sobre 100
          }
  ...
  public static Object[] obtenerResultado(boolean alMinisterio, Variable[] diferenciados, Filtro[] filtros) throws MyException {
    ...
    if (!alMinisterio) {
      resultado=KernelSPADIES.getInstance().getDesercionPorCohorte(new SFiltro[0],new SVariable[0]);
    }
    else {
