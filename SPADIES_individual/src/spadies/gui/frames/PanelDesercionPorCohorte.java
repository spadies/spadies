...
public class PanelDesercionPorCohorte extends JPanel implements ActionListener {
  ...
  public PanelDesercionPorCohorte() {
    for (int i=0; i<tam; i++) {
      ...
      for (byte[] llave:mvalsC.keySet()) {
        ...
        for (int j=0; j<tam; j++,fil++) {
          for (int k=0,tk=diferenciados.length;k<tk;k++)
            encFilas[k+1][fil+1] = SVariable.toString(diferenciados[k],llave[k]);
          for (int i=0; i<tam; i++) {
