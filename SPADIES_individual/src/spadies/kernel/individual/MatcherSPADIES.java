...
public class MatcherSPADIES {
  ...
  public MatcherSPADIES(byte[][][] pINFO_IES, double pCalificacionMinima, boolean pCompetirMejores, File pArchivoBase) throws Exception {
    INFO_IES=pINFO_IES;
  ...
  public byte[][] procesar() throws Exception {
    ...
    for (int vez=1,h1=0; vez<=VECES; vez++) {
      for (int h=0,h1T=Math.min(vez*ConstantesServer.TAM_TANDA,T1); h1<h1T; h++,h1++) {
