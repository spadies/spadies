...
public final class KernelSPADIES extends Observable {
   ...
   */
  @SuppressWarnings("unchecked")
  public Object[] getConteoPoblacion(Filtro[] filtros, Variable[] diferenciados) throws MyException {
  ...
  public Object[] getPorcentajeDesercionPorCohorte(Filtro[] filtrosIES, boolean porcentual) {
    ...
    for (IES ies:lasIES) {
      ...
      for (Estudiante e:ies.estudiantes) {
        int jI=e.getSemestrePrimiparo();
        if (jI==-1) continue;
        if (jI>=ies.n-2) continue;  // Estos estudiantes nunca se consideran desertores; afectarían el total si se consideraran
        conteoPrimiparos[mcs+jI-limInf]++;
        if (e.getEstado()==-1) {
          int i=ies.n-Long.numberOfTrailingZeros(e.getSemestresMatriculadoAlReves())-jI;
          conteo[mcs+jI-limInf][i-1]++;
        }
      }
    }
  ...
  public Object[][] getVariablesRelevantes(Filtro[] filtrosIES) throws MyException {
    ...
    for (Variable var:vars) {
      Object[] res = this.getConteoPoblacion(filtrosIES, new Variable[]{var});
      if (res==null) {
        continue;
      }
  ...
  public Object[] getTasaDesercion(Filtro[] filtros) throws MyException {
    Object [] respre = getTasaDesercion(filtros, new Variable[0]);
    return new Object[]{((Map)respre[0]).get(new byte[0]),((Map)respre[1]).get(new byte[0]),((Map)respre[2]).get(new byte[0]),respre[5],respre[6]};
  }
  public Object[] getTasaDesercion(Filtro[] filtros, Variable[] diferenciados) throws MyException {
    boolean calcularRepitencias=false;
    Integer[] codigosIESDif=null;       // Caso especial de diferenciación #1
    String[] codigosProgramasDif=null;  // Caso especial de diferenciación #2
    Filtro[] filtrosIES=Variable.filtrarFiltros(filtros,TipoVariable.TV_IES);
    ...
    for (Filtro filtro:filtros) {
      if (filtro.variable==Variable.PERIODO_MATRICULADO_PER) {
        for (Comparable byt:filtro.filtro) {
          int val = ((Byte)byt)&0xFF;
          if (val==255) continue;
          if (val>limSup0) limSup0 = val;
          if (val<limInf0) limInf0 = val;
        }
      }
    }
    ...
    IES[] lasIES=getIES(filtrosIES);
    int limsSems[]=getLimsCodigoSemestre(lasIES),
    ...
    for (IES ies:lasIES) {
      ...
      for (Estudiante e:ies.estudiantes) {
        ...
        for (int j=jI,jT=Math.min(ies.n,limSup-ies.minCodigoSemestre+1); j<jT-2; j++,matri>>>=1) if ((matri&1L)==1L /*&& pasaFiltros(filtrosPer,e,ies,j,repitencias)*/) {
          byte llavePer[]=llaveEst.clone(); //Sobraria
          if (j<limInf-ies.minCodigoSemestre) continue;
          //if ((j+ies.minCodigoSemestre<limInf) || (j+ies.minCodigoSemestre>limSup)) continue;
          /*for (int h:indicesDifPer) {
            Variable v=diferenciados[h];
            Rango rango=v.rango;
            Comparable ran=rango.getRango(v.getValor(e,ies,j,repitencias));
          ...
          if (e.getEstado()!=1 || e.getSemestreGrado() > j + 2) matriculadosConsiderados[ies.minCodigoSemestre+j-limInf]++; 
        }
        if ((ies.minCodigoSemestre+e.getUltimoSemestreMatriculado()+2>limSup)||
            (ies.minCodigoSemestre+e.getUltimoSemestreMatriculado()+2<limInf)) continue;
        if (e.getEstado()==-1) desertores[ies.minCodigoSemestre+e.getUltimoSemestreMatriculado()+2-limInf]++;
      }
    }
    if (tamRes>2) {
  ...
  @SuppressWarnings("unchecked")
  public Object[] getDesercionPorCohorte(Filtro[] filtros) throws MyException {
    Object[] respre = getDesercionPorCohorte(filtros, new Variable[0]);
  ...
  public Object[] getDesercionPorCohorte(Filtro[] filtros, Variable[] diferenciados) throws MyException {
    Integer[] codigosIESDif=null;       // Caso especial de diferenciación #1
    String[] codigosProgramasDif=null;  // Caso especial de diferenciación #2
    Map<Variable,Filtro> mapF=new EnumMap<Variable,Filtro>(Variable.class);
    for (Filtro ft:filtros) mapF.put(ft.variable,ft);
    ...
    int tamRes = limSup - limInf+1;
    if (limInf==Integer.MAX_VALUE) return null;
    //int conteoPrimiparos[]=new int[tamRes],conteo[][]=new int[tamRes][tamRes];
    for (IES ies:lasIES) {
      byte llaveIES[]=new byte[m];
      for (int h:indicesDifIES) {
        Variable v=diferenciados[h];
      ...
      for (Estudiante e:ies.estudiantes) {
        int jI=e.getSemestrePrimiparo();
        if (jI==-1 || (mcs+jI<limInf) || (mcs+jI>limSup) || !pasaFiltros(filtrosEst,e,ies)) continue;
        byte llaveEst[]=llaveIES.clone();
        for (int h:indicesDifEst) {
          Variable v=diferenciados[h];
          Rango rango=v.rango;
        ...
        if (jI>=ies.n-2) continue;  // Estos estudiantes nunca se consideran desertores; afectarían el total si se consideraran
        int[] conteoPrimiparos = conteosPrimiparos.get(llaveEst);
        if (conteoPrimiparos==null) conteosPrimiparos.put(llaveEst, conteoPrimiparos = new int[tamRes]);
        int[][] conteo = conteos.get(llaveEst);
        if (conteo==null) conteos.put(llaveEst, conteo = new int[tamRes][tamRes]);
        conteoPrimiparos[mcs+jI-limInf]++;
    ...
    return new Object[]{mres,mresP,mresC,new int[]{limInf,limSup},codigosIESDif,codigosProgramasDif};
  }
  ...
  public Object[] getGradoCohorteAcumulado(Filtro[] filtros, Variable[] diferenciados) throws MyException {
    return getCohorteEstadoAcumulado(filtros,diferenciados,(byte)1);
  }
  public Object[] getDesercion(Filtro[] filtros, Variable[] diferenciados) throws MyException {
    return getCohorteEstadoAcumulado(filtros,diferenciados,(byte)-1);
  }
  public Object[] getCohorteEstadoAcumulado(Filtro[] filtros, Variable[] diferenciados, final byte estado) throws MyException {
    Integer[] codigosIESDif=null;       // Caso especial de diferenciación #1
    String[] codigosProgramasDif=null;  // Caso especial de diferenciación #2
    Filtro[] filtrosIES=Variable.filtrarFiltros(filtros,TipoVariable.TV_IES);
    Filtro[] filtrosEst=Variable.filtrarFiltros(filtros,TipoVariable.TV_ESTUDIANTE);
    ...
    {
      Map<Variable,Filtro> mapF=new EnumMap<Variable,Filtro>(Variable.class);
      for (Filtro ft:filtrosIES) mapF.put(ft.variable,ft);
      for (Filtro ft:filtrosEst) mapF.put(ft.variable,ft);
      for (Variable v:diferenciados) if (v==Variable.CODIGO_IES || v==Variable.PROGRAMA_EST) {
        Filtro ft=mapF.get(v);
        Comparable[] keys=(ft==null)?Item.getKeys(v.items):ft.filtro;
        int t=keys.length;
    ...
    for (IES ies:lasIES) {
      ...
      for (Estudiante e:ies.estudiantes) {
        ...
        {
          int datosRef[]=serieRef.get(llaveEst);//Potencial de desertores con esas caracteristicas en cada numero de periodo.
          if (datosRef==null) serieRef.put(llaveEst,datosRef=new int[tam]);
          for (int i=0,iT=ies.n-jI+(estado==-1?-2:0); i<iT; i++) datosRef[i]++;
        }
        if (e.getEstado()==estado) {
          int serieDatos[]=resC.get(llaveEst);
          if (serieDatos==null) {
            resC.put(llaveEst,serieDatos=new int[tam]);
            if (limtser && resC.size()>120) throw new MyException("Los criterios de diferenciación suministrados arrojan más de 120 series de datos. Refine la consulta.");
          }
          int jUlt = estado==1?e.getSemestreGrado():e.getUltimoSemestreMatriculado();
          for (int j=jUlt,jT=ies.n+(estado==-1?-2:0); j<jT; j++) serieDatos[j-jI]++;
        }
      }
    }
    if (resC.isEmpty()) return null;
